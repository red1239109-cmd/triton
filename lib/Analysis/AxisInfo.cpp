...
private:
  ...
  int64_t getDivisibility(arith::MulIOp op, const AxisInfo &lhs,
    ...
    if (lhs.getContiguity(dim) > 1 && rhs.getConstantValue() != 1) {
      // If the operand is contiguous (e.g., [base, base+1, ...]), the
      // divisibility of the entire sequence drops to 1, regardless of the
      // base's divisibility.
      // Example: Input [4, 5, 6, 7]. Base 4 is divisible by 4.
      // If we preserve divisibility=4 and multiply by 2, we might
      // incorrectly infer result divisibility as 8.
      // Real result: [8, 10, 12, 14] (GCD=2).
      // Therefore, we explicitly reset input divisibility to 1 for
      // contiguous dims to prevent over-estimating the alignment.
      lhsDivisibility = 1;
    }
    auto rhsDivisibility = rhs.getDivisibility(dim);
